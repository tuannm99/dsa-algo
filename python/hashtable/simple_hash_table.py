from __future__ import annotations

from typing import Generic, TypeVar, cast

from ..ds_types import KVVisitFunc
from .hashtable import Config, HashTable

K = TypeVar("K")
V = TypeVar("V")


class SimpleHashTable(Generic[K, V], HashTable[K, V]):

    def __init__(self, config: Config[K, V]) -> None:
        self._config = config
        self._capacity = max(1, config.initial_capacity)
        self._items: list[tuple[K, V]] = []

    def _index(self, key: K) -> int:
        for index, (item_key, _) in enumerate(self._items):
            if self._config.compare_key(item_key, key) == 0:
                return index
        return -1

    def size(self) -> int:
        return len(self._items)

    def capacity(self) -> int:
        return self._capacity

    def is_empty(self) -> bool:
        return not self._items

    def set(self, key: K, value: V) -> bool:
        index = self._index(key)
        if index >= 0:
            self._items[index] = (self._items[index][0], value)
        else:
            self._items.append((key, value))
        return True

    def get(self, key: K) -> tuple[V, bool]:
        index = self._index(key)
        if index < 0:
            return cast(V, None), False
        return self._items[index][1], True

    def contains(self, key: K) -> bool:
        return self._index(key) >= 0

    def delete(self, key: K) -> tuple[K, V, bool]:
        index = self._index(key)
        if index < 0:
            return cast(K, None), cast(V, None), False
        item_key, value = self._items.pop(index)
        return item_key, value, True

    def resize(self, new_capacity: int) -> bool:
        self._capacity = max(1, new_capacity, len(self._items))
        return True

    def clear(self) -> None:
        if self._config.free_key is not None:
            for key, _ in self._items:
                self._config.free_key(key)
        if self._config.free_value is not None:
            for _, value in self._items:
                self._config.free_value(value)
        self._items.clear()

    def for_each(self, visit: KVVisitFunc[K, V]) -> None:
        for key, value in self._items:
            visit(key, value)
