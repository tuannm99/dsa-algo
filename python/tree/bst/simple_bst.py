...
from .bst import BST, Config

K = TypeVar("K")
V = TypeVar("V")


class SimpleBST(Generic[K, V], BST[K, V]):
    ...
    def pre_order(self, visit: KVVisitFunc[K, V]) -> None:
        self._map.traverse(visit)

    def post_order(self, visit: KVVisitFunc[K, V]) -> None:
        self._map.traverse(visit)
